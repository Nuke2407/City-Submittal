    ...
    try:
        with open("save_data/app_instance_state.json", "r") as file:
            ...
            if state.get("exportdocs_uploaded", False):
                app_instance.upload_button_exportdocs.configure(state="disabled", text="Export Document Uploaded")
            ...
            if state.get("table_displayed", False):
                create_missing_incorrect_data_table(app_instance, 'data/missing_incorrect_metadata_file.xlsx')
        ...
        if str(e).find('table_data.json') != -1:
            ...
            app_instance.upload_button_exportdocs.configure(state="normal", text="Upload The Export Document - ExportDocs_Stage_1.xls")
...
def save_state(app_instance, step_name):
    ...
    state = {
        ...
        "exportdocs_uploaded": app_instance.upload_button_exportdocs.cget("state") == "disabled",
