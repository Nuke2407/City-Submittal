...
def create_missing_incorrect_data_table_stage_1(self, excel_file_path):
    ...
    self.step_one_frame.grid_rowconfigure(5, weight=1)  
...
def create_missing_incorrect_data_table_stage_2(self, excel_file_path):
    ...
    #Create a frame for table and description
    self.missing_incorrect_data_table_frame_stage_2 = customtkinter.CTkFrame(self.step_two_frame, corner_radius=0)
    self.missing_incorrect_data_table_frame_stage_2.grid(row=2, column=0, padx=20, pady=10, sticky='new')
    self.missing_incorrect_data_table_frame_stage_2.grid_rowconfigure(1, weight=1) 
    self.missing_incorrect_data_table_frame_stage_2.grid_columnconfigure(0, weight=1)
    self.step_two_frame.grid_rowconfigure(5, weight=1)  
    self.step_two_frame.grid_columnconfigure(0, weight=1)
    
    #Create a description for the table 
    self.missing_incorrect_data_table_stage_2_description = customtkinter.CTkLabel(self.missing_incorrect_data_table_frame_stage_2, text="The SQs displayed below have missing metadata. Please update in Aconex.",
                                                    font=customtkinter.CTkFont(size=15))
    self.missing_incorrect_data_table_stage_2_description.grid(row=0, column=0, sticky='n')

    #Simplify column names and create the Treeview widget
    simplified_columns = [col.replace('?', '').replace('(', '').replace(')', '').replace('/', '_').replace(' ', '_') for col in dataframe_stage_2.columns]
    self.missing_incorrect_data_table_stage_2 = ttk.Treeview(self.missing_incorrect_data_table_frame_stage_2, columns=simplified_columns, show='headings')
    self.missing_incorrect_data_table_stage_2.grid(row=1, column=0, padx=20, pady=10, sticky='nsew')

    #Create and place a vertical scrollbar
    scrollbar = ttk.Scrollbar(self.missing_incorrect_data_table_frame_stage_2, orient="vertical", command=self.missing_incorrect_data_table_stage_2.yview)
    scrollbar.grid(row=1, column=1, sticky='ns')
    self.missing_incorrect_data_table_stage_2.configure(yscrollcommand=scrollbar.set)

    #Download Button for the Excel file.
    self.download_missing_metadata_file_button_stage_2 = customtkinter.CTkButton(self.missing_incorrect_data_table_frame_stage_2, text="Download Missing Metadata Excel File", command=download_missing_incorrect_data_table_stage_2)
    ...
    sheet_selector = customtkinter.CTkOptionMenu(self.missing_incorrect_data_table_frame_stage_2, values=sheet_names_stage_2, variable=SQs_With_Missing_Data, command=update_table, corner_radius=0)
    sheet_selector.grid(row=2, column=0, sticky='wn', padx=20, pady=0)
